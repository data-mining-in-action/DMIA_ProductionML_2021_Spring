...
1) Докер:
    ...
    4) В качестве дополнительного задания попробуйте оптимизировать ваш докер-файл так, чтобы его сборка выполнялась быстрее. В качестве второй цели попробуйте уменьшить размер докер-образа (это может пригодиться вам при деплое на heroku).
