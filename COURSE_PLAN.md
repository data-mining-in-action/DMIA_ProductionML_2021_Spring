...
1) Докер:
    1) Посмотреть [пример в репозитории с паролями](https://gitlab.com/production-ml/password-complexity/-/blob/master/Dockerfile.app) и убедиться что все понятно. В частности, проверить, что понятно, что делает каждая команда, в чем разница ENV и ARG, RUN и CMD. Чтобы проверить себя, удалите случайную строчку из докер-файла и попробуйте угадать, с какой ошибкой упадет сборка/запуск докер-образа.
    2) Собрать докер-контейнер из этого образа и поднять его локально. Убедиться, что приложение работает.
    3) Упаковать в докер-образ свой сервис с паролями. Поднимите его у себя локально и убедитесь, что до приложения внутри можно достучаться и можно получить прогноз.
    4) В качестве дополнительного задания попробуйте оптимизировать ваш докер-файл так, чтобы его сборка выполнялась быстрее. В качестве второй цели попробуйте уменьшить размер докер-образа (это может пригодиться вам при деплое на heroku). Ссылки для чтения: [раз](https://medium.com/sciforce/strategies-of-docker-images-optimization-2ca9cc5719b6) и [два](https://blog.jscrambler.com/dockerfile-optimization-for-fast-builds-and-light-images/).
...
#### Домашнее задание
1) Шляпа: вам потребуется написать своего бота для Шляпы, пример [the-hat-game](https://gitlab.com/production-ml/the-hat-game). Для начала это может быть просто сервис на Flask или другом веб-фреймворке, который отвечает в нужном формате ответом-заглушкой. Попытайтесь задеплоить его на heroku и убедиться, что это получается. Затем попробуйте усложнить логику ответов, добавив word embeddings или воспользовавшись другими подходами. Имейте в виду, что по правилам использовать для обучения можно лишь предоставленные данные.
    
    Как проверить, что вы выполнили задание достаточно хорошо: откройте вашего бота на heroku и проверьте, что сервис отвечает. Отправьте запросы с помощью curl и убедитесь, что формат ответов соответствует приведенному в примерах в репозитории. Попросите членов вашей команды также это проверить. Наконец, проведите тестовые запуски игры между вашими моделями с помощью скриптов в репозитории.
1) Тесты: обложить тестами свои ML-модели и сервис - уже для шляпы (на этом же занятии мы завершаем пароли и начинаем Шляпу). Имеется в виду, что нужно написать 5+ тестов и убедиться, что они работают через вызов pytest (это пригодится при встраивании в CI). Пример можно посмотреть в репозитории [password-complexity](https://gitlab.com/production-ml/password-complexity).
...
#### Домашнее задание
1) Вам потребуется написать ML Pipeline для своего бота-игрока в Шляпу, подобный тому, что мы разбирали на семинаре. Он есть в нашем репозитории [passwords-complexity](https://gitlab.com/production-ml/password-complexity). В итоге, этот пайплайн должен запускаться скриптом run_all.sh или аналогичным на bash, Python или другом языке (можно использовать и функциональный, и ООП подход). Важно, что окружение должно создаваться автоматически - для этого может использоваться pipenv ("pipenv run my_pipeline.py") или docker ("docker run ml_pipeline:latest"). Docker предпочтительнее, поскольку позже мы будем запускать созданный докер-образ в рамках CI/CD. Если для запуска требуется определенная подготовка - например, нужно установить Docker или pipenv, напишите об этом в README своего репозитория. 

    Как проверить, что вы выполнили задание достаточно хорошо: дайте выполнить эту задачу другому члену команды по текстовой инструкции, а если он тоже в этом участвовал - то попробуйте склонировать свой репозиторий на новую машинку и запустить этот скрипт следуя вашей инструкции шаг за шагом. В процессе выполнения должны скачиваться данные, распечатываться необходимые метрики, сохраняться обученная модель.
2) Опционально - используйте [dvc](https://dvc.org/doc) и [dvc pipelines](https://dvc.org/doc/start/data-pipelines) для версионирования своих данных, сборки пайплайна вместе и его запуска. Если вы используете dvc, то в [Studio](https://viewer.iterative.ai) можно открыть ваш репозиторий и просмотреть ваши метрики: репозиторий курса можно [посмотреть тут](https://studio.iterative.ai/user/aguschin/views/password-complexity-v9tec71kjd). Для того, чтобы использовать dvc полноценно, придется использовать какой-нибудь remote storage, его можно создать в google storage в течении тестового free tier периода.
...
#### Домашнее задание
1) Воспроизводимость экспериментов: залогировать свои эксперименты Neptune / DVC + Studio. Убедиться, что они воспроизводятся на разных машинках: выкачать на новую/другую машинку репозиторий и перезапустить эксперимент, сравнить метрики.

    (Опционально) Добавить критерий обновления модели в сервисе в staging/production. Сейчас это будет просто кусочек кода, который позволит вам сравнить качество нового эксперимента с текущей версией в проде и принять решение об обновлении сервиса (позже мы встроим это в CI/CD). Как один из простых вариантов, записывать метрики текущей модели в файл, а затем сравнивать их с тем, что посчитано во время валидации новой модели.
2) (Опционально) Мониторинг: поднять прометеус/графану с помощью docker-compose в репозитории паролей. Убедиться, что метрики собираются, посмотреть на дашборды, попробовать настроить нужные метрики. Разобраться с алертами и настроить их. Перенести это в свой репозиторий по шляпе.
